  ...
  return (
    <div className="input-card">
      ...
      {travelers.length > 0 && (
        <>
          <div className="stats">
            Total: {travelers.length} | Drivers: {drivers} | Children: {children}
          </div>
          <div className="list">
            {travelers.map(traveler => (
              <div key={traveler.id} className="list-item">
                <div className="list-item-content">
                  ...
                  {traveler.isChild && <span className="driver-badge child-badge">👶 Child</span>}
                  {traveler.canDrive && <span className="driver-badge">🚗 Driver</span>}
                </div>
                <div className="list-item-actions">
                  {!traveler.isChild && (
                    <button
                      ...
                      title={traveler.canDrive ? "Remove driver status" : "Make driver"}
                    >
                      {traveler.canDrive ? '🚗' : '➕'}
                    </button>
                  )}
                  <button onClick={() => removeTraveler(traveler.id)} className="remove-button">×</button>
                </div>
              </div>
            ))}
          </div>
